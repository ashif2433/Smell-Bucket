...
@section('content')
    <div class="d-flex justify-content-between">
        <div>
            <h6 class="mb-0 text-uppercase">Top Notification Slider</h6>
        </div>
        {{-- <div>
            <a class="btn btn-dark" href="{{ route('admin.btobcreate') }}"><i class="fadeIn animated bx bx-plus"></i> Add New</a>
        </div> --}}
    </div>
    <hr />
    <div class="row">
                    ...
                    <div class="table-responsive">
                        <table id="example" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                ...
                                @foreach ($topmar as $item)
                                    <tr>
                                        <td><input type="checkbox" class="select-row" value="{{ $item->id }}"></td>
                                        <td>
                                            <p>{{ \Illuminate\Support\Str::words($item->details, 20, '...') }}</p>
                                        </td>
                                        <td>
                                            <a class="btn btn-sm btn-info" href="{{ route('admin.topmaredit', $item->id) }}"><i class="fadeIn animated bx bx-pencil"></i></a>
                                            {{-- <a class="btn btn-sm btn-danger" href="{{ route('admin.btobdelete', $item->id) }}"><i class="fadeIn animated bx bx-trash-alt"></i></a> --}}
                                        </td>
                                    </tr>
                                @endforeach
