...
@section('content')
    ...
    <div class="d-flex justify-content-between">
        <div>
            <h6 class="mb-0 text-uppercase">Approved Orders</h6>
        </div>
    </div>
    <hr />
    <div class="row">
    ...
    document.addEventListener("DOMContentLoaded", function () {
        document.querySelectorAll('.print-btn').forEach(function (button) {
            button.addEventListener('click', function () {
                ...
                printWindow.document.write('<h1 style="text-align: center;">{{ config('app.name') }}</h1>');
                printWindow.document.write('<h2>Order Details</h2>');
                printWindow.document.write('<table>');
                printWindow.document.write('<thead>' + thead.innerHTML + '</thead>'); // Add table headers
                printWindow.document.write('<tbody><tr>' + clonedRow.innerHTML + '</tr></tbody>'); // Add selected row
                printWindow.document.write('</table>');

                // Add Signature Section
                printWindow.document.write('<div class="signature"><div>Authorized Signature</div></div>');
                ...
                doc.autoTable({
                    head: [Array.from(thead.querySelectorAll('th')).map(th => th.textContent)],
                    body: [[...clonedRow.querySelectorAll('td')].map(td => td.textContent)],
                    startY: 30,
                    theme: 'grid',
                });

                // Add signature section
                doc.text('Authorized Signature', 180, doc.lastAutoTable.finalY + 20, { align: 'right' });

                // Generate a random number for the filename
                let randomNumber = Math.floor(Math.random() * 1000000);  // Random number between 0 and 1,000,000
