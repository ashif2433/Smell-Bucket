...
@section('content')
    <div class="d-flex justify-content-between">
        <div>
            <h6 class="mb-0 text-uppercase">All Terms</h6>
        </div>
        <div>
            <a class="btn btn-dark" href="{{ route('admin.termscreate') }}"><i class="fadeIn animated bx bx-plus"></i> Add New</a>
        </div>
    </div>
    <hr />
    <div class="row">
        <div class="col">
            <div class="card">
                    ...
                    <div class="table-responsive">
                        <table id="example" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                ...
                                @foreach ($terms as $product)
                                    <tr>
                                        <td><input type="checkbox" class="select-row" value="{{ $product->id }}"></td>
                                        <td>{{ \Illuminate\Support\Str::words($product->details, 10, '..') }}</td>
                                        <td>
                                            <a class="btn btn-sm btn-info" href="{{ route('admin.termsedit', $product->id) }}"><i class="fadeIn animated bx bx-pencil"></i></a>
                                            <a class="btn btn-sm btn-danger" href="{{ route('admin.termsdelete', $product->id) }}"><i class="fadeIn animated bx bx-trash-alt"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                ...
                {{-- <marquee style="color: red">there are 3 Default FAQ.</marquee> --}}
            </div>
        </div>
    </div>
@endsection
