...
class CustomerController extends Controller
{
    ...
    //     $credentials = filter_var($request->phone_email, FILTER_VALIDATE_EMAIL)
    //         ? ['email' => $request->phone_email, 'password' => $request->password]
    //         : ['phone' => $request->phone_email, 'password' => $request->password];

    //     // DEBUG: Check if user exists
    //     $customer = Customer::where('email', $request->phone_email)
    //                         ->orWhere('phone', $request->phone_email)
    //                         ->first();

    //     // dd(Customer::first()->password);
    ...
    //         return back()->with('login_error', 'Incorrect password');
    //     }

    //     // DEBUG: Check if Auth::attempt() works
    //     if (!Auth::guard('customer')->attempt($credentials)) {
    //         return back()->with('login_error', 'Auth attempt failed');
    //     }
    ...
    public function address_edit($id)
    {
        $customer = Customer::find(session('customer_id'));
        // dd($customer);
        return view('frontend.customer.add_address',compact('customer'));
    }
    ...
    public function address_update2($id, Request $request)
    {
        $request->validate([
            'address2' => 'nullable',
        ]);
    ...
    public function myorder($phone)
    {
        ...
        $customer = Customer::where('phone', $phone)->first();
        // $orders = Order::all();
        $orders = Order::orderByRaw("FIELD(delivery_status, 'Pending', 'Approved', 'Shipping' , 'Delivered' , 'Cancelled')")->latest()->get();
    ...
    public function addReferral(Request $request)
    {
        try {
            ...
            if ($customer->id == $referrer->id) {
                return response()->json(['success' => false, 'message' => 'You cannot refer yourself.']);
            }

            $customer->referral_by = $referrer->id;
            $customer->save();
