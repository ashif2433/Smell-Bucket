...
@section('content')
    <div class="d-flex justify-content-between">
        ...
        <div>
            <a class="btn btn-dark" href="{{ route('admin.faqcreate') }}"><i class="fadeIn animated bx bx-plus"></i> Add New</a>
        </div>
    </div>
    <hr />
    <div class="row">
                    ...
                    <div class="table-responsive">
                        <table id="example" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                ...
                                @foreach ($faq as $product)
                                    <tr>
                                        <td><input type="checkbox" class="select-row" value="{{ $product->id }}"></td>
                                        <td>{{ $product->question }}</td>
                                        <td>{{ \Illuminate\Support\Str::words($product->answer, 10, '..') }}</td>
                                        <td>
                                            <a class="btn btn-sm btn-info" href="{{ route('admin.faqedit', $product->id) }}"><i class="fadeIn animated bx bx-pencil"></i></a>
                                            <a class="btn btn-sm btn-danger" href="{{ route('admin.faqdelete', $product->id) }}"><i class="fadeIn animated bx bx-trash-alt"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
